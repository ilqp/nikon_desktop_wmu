...
def performRename(formatString, parmsDict):
    ...
    specifierTranslationDict['capturedate_dow'] = str(
        captureTimeStruct.tm_wday + 1)  # date captured day of week (numeric, 1=Monday)
    specifierTranslationDict['capturedate_woy'] = time.strftime("%W",
                                                                captureTimeStruct)  # date captured week of year (numeric, Monday first day of week)
    specifierTranslationDict['capturedate_month'] = time.strftime("%B", captureTimeStruct)  # date captured month (text)
    specifierTranslationDict['capturedate_dayofweek'] = time.strftime("%A",
                                                                      captureTimeStruct)  # date captured day of week (text)
    specifierTranslationDict['capturedate_season'] = dayOfYearToSeason(
        captureTimeStruct.tm_yday)  # date captured season (text)
    ...
    specifierTranslationDict['dldate_dow'] = str(
        downloadTimeStruct.tm_wday + 1)  # date downloaded day of week (numeric, 1=Monday)
    specifierTranslationDict['dldate_woy'] = time.strftime("%W",
                                                           downloadTimeStruct)  # date downloaded week of year (numeric, Monday first day of week)
    specifierTranslationDict['dldate_month'] = time.strftime("%B", downloadTimeStruct)  # date downloaded month (text)
    specifierTranslationDict['dldate_dayofweek'] = time.strftime("%A",
                                                                 downloadTimeStruct)  # date downloaded day of week (text)
    specifierTranslationDict['dldate_season'] = dayOfYearToSeason(
        downloadTimeStruct.tm_yday)  # date downloaded season (text)
    ...
    specifierTranslationDict['slotnumber'] = str(
        parmsDict['slotnumber'])  # camera media slot # file was downloaded from
    ...
    specifierTranslationDict['dlnum_lifetime'] = "{:04d}".format(
        parmsDict['dlnum_lifetime'])  # download number lifetime for this model/serial
    ...
    while formatStringPos < formatStringLen:
        ...
        specifierForReporting = formatString[
                                nextSpecifierStartPos:nextSpecifierEndPos + 1]  # for use in reporting errors, entire specifier in original case including enclosing @@
        specifierWithArgs = formatString[
                            nextSpecifierStartPos + 1:nextSpecifierEndPos]  # the entire specifier with optional args included (everything between @@)
        ...
        if specifierWithArgsLowercase.find('replace', 0, 7) != -1:
            ...
            if len(replaceList) == 1:
                raise GenerateReplacementNameException(
                    "No search string specified after specifier {:s} at character #{:d}".format(specifierForReporting,
                                                                                                nextSpecifierStartPos))
            ...
            if len(replaceList) > 3:
                raise GenerateReplacementNameException(
                    "Too many fields for specifier {:s} at character #{:d}".format(specifierForReporting,
                                                                                   nextSpecifierStartPos))
        ...
        if len(specifierListLowercase) > 4:
            raise GenerateReplacementNameException(
                "Too many subscripts to specifier {:s} at character #{:d}".format(specifierForReporting,
                                                                                  nextSpecifierStartPos))
...
def testModule():
    ...
    print(performRename("pf=@pf@, filename=@filename@, root=@filename_root@, ext=@filename_ext@, dir=@CAMERAFOLDER@",
                        renameDict))
    print(performRename(
