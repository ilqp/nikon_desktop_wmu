...
class DownloadStatsStruct:
    ...
    def reportDownloadStats(self, fDontPrintStatsIfNoFilesDownloaded=False):
        ...
        if g.dlstats.countFilesSkippedDueToFileExistingLocally:
            applog_i("{:d} files skipped because they already existed in output directory".format(
...
class MtpObject(LinkedListObj):
    ...
    def __init__(self, mtpObjectHandle, mtpObjectInfo):
        ...
        if self.mtpObjectInfo.captureDateStr:  # there is a non-empty capture date string
            ...
            if self.mtpObjectInfo.associationType == MTP_OBJASSOC_GenericFolder:
                if len(self.mtpObjectInfo.filename) == 10 and self.mtpObjectInfo.filename[4] == '-' and \
                                self.mtpObjectInfo.filename[7] == '-':
    ...
    def getImmediateDirectory(self):  # gets immediate camera directory that this object is in. ex: "100NC1J4"
        ...
        if objHandleDirectory not in MtpObject.__MtpObjects_ObjectHandleDict:
            applog_d("getImmediateDirectory(): Unable to locate parent object for {:s}, parent=0x{:08x}".format(
    ...
    def genFullPathStr(
            self):  # builds full path string to this object on camera, including filename itself. Ex: "DCIM\100NC1J4\DSC_2266.NEF"
        ...
        while (objHandleAncestorDirectory != 0):
            if objHandleAncestorDirectory not in MtpObject.__MtpObjects_ObjectHandleDict:
                ...
                applog_d("genFullPathStr(): Unable to locate parent object for {:s}, parent=0x{:08x}".format(
                    self.mtpObjectInfo.filename, objHandleDirectory))
                return pathStr
    ...
    def __str__(self):  # generates string description of object
        ...
        s += "    thumbPixDimensions = " + str(self.mtpObjectInfo.thumbPixWidth) + "x" + str(
            self.mtpObjectInfo.thumbPixHeight) + "\n"
        s += "    imagePixDimensions = " + str(self.mtpObjectInfo.imagePixWidth) + "x" + str(
            self.mtpObjectInfo.imagePixHeight) + "\n"
...
def verifyPythonVersion():
    if sys.version_info.major == 2:
        if sys.version_info.minor < 7:
            ...
            applog_i("Version running is {:d}.{:d}.{:d}, app was tested on 2.7.x".format(sys.version_info.major,
                                                                                         sys.version_info.minor,
                                                                                         sys.version_info.micro))
    elif sys.version_info.major == 3:
        if sys.version_info.minor < 4:
            applog_i("Warning: You are running a Python 3.x version older than app was tested with.")
            applog_i("Version running is {:d}.{:d}.{:d}, app was tested on 3.4.x".format(sys.version_info.major,
                                                                                         sys.version_info.minor,
                                                                                         sys.version_info.micro))
...
def establishAppEnvironment():
    ...
    if g.isFrozen and g.isWin32:
        if os.getenv('LOCALAPPDATA'):
            g.appDataDir = os.path.join(os.getenv('LOCALAPPDATA'),
                                        "airnef\\appdata")  # typically C:\Users\<username>\AppData\Local\airnef\appdata
        ...
        if userHomeDir:
            ...
            if os.path.exists(
                    applicationSupportDir):  # probably not necessary to check existence since every system should have this directory
...
def verifyIntegerArgRange(argNameStr, minAllowedValue, maxAllowedValue):
    if g.args[argNameStr] != None:
        if g.args[argNameStr] < minAllowedValue or g.args[argNameStr] > maxAllowedValue:
            applog_e("Invalid value for --{:s}: valid range is [{:d}-{:d}]".format(argNameStr, minAllowedValue,
                                                                                   maxAllowedValue))
...
class ArgumentParserError(
    Exception): pass  # from http://stackoverflow.com/questions/14728376/i-want-python-argparse-to-throw-an-exception-rather-than-usage
...
def processCmdLine():
    ...
    parser.add_argument('--ipaddress', type=str.lower, help='IP address of camera. Default is "%(default)s"',
                        default='192.168.1.1', metavar="addr", required=False)
    parser.add_argument('--action', type=str.lower,
                        choices=['getfiles', 'getsmallthumbs', 'getlargethumbs', 'listfiles'],
                        help='Program action. Default is "%(default)s"', default='getfiles', required=False)
    parser.add_argument('--realtimedownload', type=str.lower, choices=['disabled', 'afternormal', 'only'],
                        ...
                        default='disabled', required=False)
    parser.add_argument('--extlist',
                        ...
                        default=None, nargs='+', metavar='extension', required=False)
    ...
    parser.add_argument('--outputdir', type=str,
                        ...
                        default=None, metavar="path", required=False)
    parser.add_argument('--ifexists', type=str.lower, choices=['uniquename', 'skip', 'overwrite', 'prompt', 'exit'],
                        help='Action to take if file with same name already exists. Default is "%(default)s"',
                        default='uniquename', required=False)
    parser.add_argument('--downloadhistory', type=str.lower, choices=['skipfiles', 'ignore', 'clear'],
                        ...
                        default='skipfiles', required=False)
    parser.add_argument('--onlyfolders',
                        ...
                        default=None, nargs='+', metavar="camera_folder", required=False)
    parser.add_argument('--excludefolders',
                        help='Exclude image/file(s) existing in specified camera folders.. Ex: \"--excludefolders 103D7200\". Default is no exclusions.',
                        default=None, nargs='+', metavar="camera_folder", required=False)
    parser.add_argument('--filenamespec', type=str,
                        help='Optionally rename files using dynamic renaming engine. See online help for documentation on \'spec\'',
                        default=None, metavar="spec", required=False)
    parser.add_argument('--dirnamespec', type=str,
                        help='Optionally name directories using dynamic renaming engine. See online help for documentation on \'spec\'',
                        default=None, metavar="spec", required=False)
    parser.add_argument('--transferorder', type=str.lower, choices=['oldestfirst', 'newestfirst'],
                        help='Transfer oldest or newest files first. Default is "%(default)s"', default='oldestfirst',
                        required=False)
    parser.add_argument('--slot', type=str.lower,
                        help='Card slot on camera to read from. Default is "%(default)s", which means first populated slot',
                        choices=['firstfound', 'first', 'second', 'both'], default='firstfound', required=False)
    parser.add_argument('--cameratransferlist', type=str.lower, choices=['useifavail', 'exitifnotavail', 'ignore'],
                        help='Decide how to handle images selected on camera. Default is "%(default)s"',
                        default='useifavail', required=False)
    parser.add_argument('--downloadexec', help='Launch application for each file downloaded', default=None, nargs='+',
                        metavar=('executable', 'arguments'), required=False)
    parser.add_argument('--downloadexec_extlist',
                        help='Type of files(s) by extension on wich to perform --downloadexec on. Default is all file types',
                        default=None, nargs='+', metavar='extension', required=False)
    parser.add_argument('--downloadexec_options',
                        ...
                        default=[], nargs='+', metavar='option', required=False)
    parser.add_argument('--realtimepollsecs', type=int,
                        help='How often camera is polled for new images in realtime mode, in seconds. Default is every %(default)s seconds',
                        default=3, metavar="seconds", required=False)
    parser.add_argument('--logginglevel', type=str.lower, choices=['normal', 'verbose', 'debug'],
    ...
    parser.add_argument('--printstackframes', help=argparse.SUPPRESS, type=str.lower, choices=['no', 'yes'],
                        default='no', required=False)
    parser.add_argument('--mtpobjcache', type=str.lower,
                        choices=['enabled', 'writeonly', 'readonly', 'verify', 'disabled'], help=argparse.SUPPRESS,
                        default='enabled', required=False)
    parser.add_argument('--mtpobjcache_maxagemins', help=argparse.SUPPRESS, type=int, default=0,
                        required=False)  # default is 0=indefinite (never invalidate based on age)
    parser.add_argument('--maxgetobjtransfersizekb', help=argparse.SUPPRESS, type=int,
                        default=DEFAULT_MAX_KB_PER_GET_OBJECT_REQUEST, required=False)
    parser.add_argument('--maxgetobjbuffersizekb', help=argparse.SUPPRESS, type=int,
                        default=DEFAULT_MAX_KB_TO_BUFFER_FOR_GET_OBJECT_REQUESTS, required=False)
    parser.add_argument('--initcmdreq_guid', help=argparse.SUPPRESS, type=str.lower,
                        default='0x7766554433221100-0x0000000000009988',
                        required=False)  # GUID order in string is high-low
    parser.add_argument('--initcmdreq_hostname', help=argparse.SUPPRESS, type=str, default='airnef', required=False)
    parser.add_argument('--initcmdreq_hostver', help=argparse.SUPPRESS, type=conver_int_auto_radix, default=0x00010000,
                        required=False)
    parser.add_argument('--opensessionid', help=argparse.SUPPRESS, type=conver_int_auto_radix, default=None,
                        required=False)
    parser.add_argument('--maxclockdeltabeforesync', help=argparse.SUPPRESS, type=str.lower, default='5',
                        required=False)
    parser.add_argument('--camerasleepwhendone', help=argparse.SUPPRESS, type=str.lower, choices=['no', 'yes'],
                        default='yes', required=False)
    parser.add_argument('--sonyuniquecmdsenable', help=argparse.SUPPRESS, type=conver_int_auto_radix,
                        default=SONY_UNQIUECMD_ENABLE_SENDING_MSG, required=False)
    parser.add_argument('--suppressdupconnecterrmsgs', help=argparse.SUPPRESS, type=str.lower, choices=['no', 'yes'],
                        default='yes', required=False)
    parser.add_argument('--rtd_pollingmethod', help=argparse.SUPPRESS, type=int, default=None, required=False)
    parser.add_argument('--rtd_mtppollingmethod_newobjdetection', help=argparse.SUPPRESS, type=str.lower,
                        choices=['objlist', 'numobjs'], default='objlist', required=False)
    parser.add_argument('--rtd_maxsecsbeforeforceinitialobjlistget', help=argparse.SUPPRESS, type=int, default=5,
                        required=False)
    parser.add_argument('--ssdp_discoveryattempts', help=argparse.SUPPRESS, type=int, default=3, required=False)
    parser.add_argument('--ssdp_discoverytimeoutsecsperattempt', help=argparse.SUPPRESS, type=int, default=2,
                        required=False)
    parser.add_argument('--ssdp_discoveryflags', help=argparse.SUPPRESS, type=conver_int_auto_radix, default=None,
                        required=False)
    ...
    if os.path.exists(defaultArgFilename):
        sys.argv.insert(1,
                        "!" + defaultArgFilename)  # insert as first arg (past script name), so that the options in the file can still be overriden by user-entered cmd line options
    ...
    g.fileTransferOrder = FILE_TRANSFER_ORDER_OLDEST_FIRST if g.args[
                                                                  'transferorder'] == 'oldestfirst' else FILE_TRANSFER_ORDER_NEWEST_FIRST
    ...
    if g.args['logginglevel'] == 'normal':
        ...
        applog_set_loggingFlags(
            APPLOGF_LEVEL_INFORMATIONAL | APPLOGF_LEVEL_ERROR | APPLOGF_LEVEL_WARNING | APPLOGF_LEVEL_VERBOSE | APPLOGF_LEVEL_DEBUG)
...
def parseMtpCountedList(data, elementSizeInBytes):
    ...
    for entryIndex in xrange(countEntries):
        (entry,) = struct.unpack('<' + elementSizeToUnpackStr[elementSizeInBytes],
                                 data[offset:offset + elementSizeInBytes])
...
def parseMtpStorageInfo(data):
    (storageType, fileSystemType, accessCapability, maxCapacityBytes, freeSpaceBytes, freeSpaceInImages,
     storageDescription) = struct.unpack('<HHHQQIB', data[0:27])
...
def parseMtpDeviceInfo(data):
    ...
    serialNumberStr = removeLeadingCharsFromStr(serialNumberStr,
                                                {' ', '0'})  # remove leading spaces/zeros from serial number

    return MtpDeviceInfoTuple(standardVersion, vendorExtensionID, vendorExtensionVersion, vendorExtensionDescStr, \
                              set(operationsSupportedList), set(eventsSupportedList),
...
def getMtpObjectInfo(objHandle, fRetryErrors=True):
    ...
    while True:
        try:
            ...
            applog_d("getMtpObjectInfo() failed: {:s}, elaspsedTimeSecs={:.2f}".format(getMtpRespDesc(e.mtpRespCode),
                                                                                       elapsedTimeSecs))
...
def ssdpDiscoverCameraIpAddress():
    ...
    try:
        ssdpMessage = ssdp.discover(ssdpServiceNames,
                                    lambda ssdpMessage: ssdp.extractIpAddressFromSSDPMessage(ssdpMessage) != None, \
                                    g.args['ssdp_discoveryattempts'], g.args['ssdp_discoverytimeoutsecsperattempt'],
                                    g.args['ssdp_discoveryflags'], g.args['ssdp_addmulticastif'])
        if ssdpMessage == None:
...
def startMtpSession():
    ...
    #
    g.socketPrimary = mtpwifi.openConnection(ipAddressStr, True, g.args['connecttimeout'],
                                             g.args['socketreadwritetimeout'])

    #
    # get session ID
    #
    data = mtpwifi.sendInitCmdReq(g.socketPrimary, (guidHigh, guidLow), g.args['initcmdreq_hostname'],
                                  g.args['initcmdreq_hostver'])
    ...
    #
    g.socketEvents = mtpwifi.openConnection(ipAddressStr, False, g.args['connecttimeout'],
                                            g.args['socketreadwritetimeout'])
    ...
    while True:
        try:
            ...
            if not fSessionIdFromCmdLine and e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR:
                applog_d("MTP_OP_OpenSession failed: {:s} for sessionID 0x{:08x}".format(getMtpRespDesc(e.mtpRespCode),
                                                                                         g.sessionId))
                if g.sessionId != 0x1:
...
def endMtpSession():
    ...
    if secsElapsedSinceOpenSession < 1:
        applog_d("endMtpSession(): Delaying for {:.2f} seconds to work around Nikon bug".format(
...
def getMtpDeviceInfo():
        ...
        if mtpDeviceInfo != g.mtpDeviceInfo:
            applog_e(
                "Discovered different camera during retry invocation. Orig camera was Model \"{:s}\", S/N \":{:s}\", New camera is \"{:s}\", S/N \":{:s}\"".format( \
                    g.mtpDeviceInfo.modelStr, g.mtpDeviceInfo.serialNumberStr, mtpDeviceInfo.modelStr,
    ...
    #
    g.cameraLocalMetadataPathAndRootName = os.path.join(g.appDataDir, "{:s}-SN{:s}".format(g.mtpDeviceInfo.modelStr,
...
def selectMtpStorageId():
    ...
    applog_d("  storageId to be used for this invocation: {:08x} [cardsPresentBitmap=0x{:04x}]".format(storageId,
                                                                                                       cardsPresentBitmap))
    ...
    if g.storageId != MTP_STORAGEID_ALL_CARDS:
        ...
        for i in xrange(0, countCardSlots):
            ...
            if (storageId & MTP_STORAGEID_PresenceBit):
                mtpTcpCmdResult = mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_GetStorageInfo,
                                                    struct.pack('<I', storageId))
                mtpStorageInfo = parseMtpStorageInfo(mtpTcpCmdResult.dataReceived)
                applog_d(mtpStorageInfo)
                g.mtpStorageInfoList.append(mtpStorageInfo)
...
def saveMtpObjectsToDiskCache():
    ...
    hashOfMtpObjectInfoCacheTuple = hashlib.sha512(
        mtpObjectInfoCacheAsPickleBytes).hexdigest()  # generate a SHA-512 hash digest, represented in hex ASCII
    ...
    try:
        ...
        fMtpObjectCache.write(
            mtpObjectInfoCacheAsPickleBytes)  # write the tuple. note we're using the already-pickled data instead of calling dump(). this is for performance
        pickle.dump(hashOfMtpObjectInfoCacheTuple, fMtpObjectCache)  # write the SHA-512 hash digest
        fMtpObjectCache.close()
    except IOError as e:
        if fMtpObjectCache:
            fMtpObjectCache.close()
        applog_e("I/O error writing obj hash to {:s}, cache will not be available next session: {:s}".format(
            mtpObjectInfoCacheFilename, str(e)))
        deleteFileIgnoreErrors(
            mtpObjectInfoCacheFilename)  # delete cache in case some (partial) data was written before exception
...
def loadMtpObjectInfoCacheFromDisk():
    ...
    if os.path.exists(mtpObjectInfoCacheFilename):
        ...
        try:
            ...
            if fMtpObjectCache:
                fMtpObjectCache.close()
            applog_e("Non-I/O error reading obj hash from {:s}. Cache will be discarded. (err={:s})".format(
        ...
        applog_v("MTP Object cache has {:d} objects, age is {:s}". \
                 format(len(mtpObjectInfoCacheTuple.mtpObjectInfoList),
                        str(datetime.timedelta(seconds=cacheAgeSeconds))))
        if isDebugLog():
            ...
            for i in xrange(len(mtpObjectInfoCacheTuple.mtpObjectInfoList)):
                applog_d("Entry {:4d}, handle = 0x{:08x}: {:s}".format(i, mtpObjectInfoCacheTuple.objHandlesList[i],
                                                                       str(mtpObjectInfoCacheTuple.mtpObjectInfoList[
                                                                               i])))
        ...
        if g.args['mtpobjcache_maxagemins'] != 0 and cacheAgeSeconds / 60 >= g.args['mtpobjcache_maxagemins']:
            applog_v("Discarding MTP object cache due to age (max age is {:s})".format(
...
def loadAndValidateMtpObjectInfoCacheFromDisk(objHandlesFromCameraList):
    ...
    for nObjIndex in xrange(0, len(mtpObjectInfoCacheTuple.mtpObjectInfoList)):
        ...
        if objHandle not in objHandlesFromCameraSet:
            applog_d("MTP obj handle 0x{:08x} for cache directory object \"{:s}\" does not exist".format(objHandle,
                                                                                                         cachedMtpObjectInfo.filename))
        ...
        #
        try:
            applog_d("Validating MTP obj cache directory object \"{:s}\" on handle 0x{:08x}".format(
...
#
def createMtpObjectFromHandle(objHandle, createMtpObjectStatsStruct=None, cachedMtpObjectInfoListDict=None,
                              fFindAndCreateAntecendentDirs=True, mtpObjectInfo=None):
    ...
    if mtpObj:
            ...
            if fIsInCache and g.args['mtpobjcache'] != 'verify':
                ...
                if fIsInCache:
                    ...
                    if mtpObjectInfo != cachedMtpObjectInfoListDict[objHandle]:
                        applog_e(
                            "Found MTP object cache mismatch for \"{:s}\" vs  \"{:s}\"".format(mtpObjectInfo.filename,
                                                                                               cachedMtpObjectInfoListDict[
                                                                                                   objHandle].filename))
        ...
        #
        if fFindAndCreateAntecendentDirs and mtpObjectInfo.parentObject and MtpObject.getByMtpObjectHandle(
                mtpObjectInfo.parentObject) == None:
            applog_d("Recursing to get parent dir of \"{:s}\" - objHandle=0x{:08x}, parent=0x{:08x}".format(
                mtpObjectInfo.filename, objHandle, mtpObjectInfo.parentObject))
            createMtpObjectFromHandle(mtpObjectInfo.parentObject, createMtpObjectStatsStruct,
                                      cachedMtpObjectInfoListDict, fFindAndCreateAntecendentDirs)
...
#
def createMtpObjectsFromHandleList(objHandlesList, createMtpObjectStatsStruct=None, cachedMtpObjectInfoListDict=None,
    ...
    for nObjIndex in xrange(0, numObjectHandles):
        ...
        createMtpObjectFromHandle(objHandlesList[nObjIndex], createMtpObjectStatsStruct, cachedMtpObjectInfoListDict,
...
def buildMtpObjects(fReportObjectTotals=True):
    ...
    if g.args['cameratransferlist'] != 'ignore':
        ...
        if MTP_OP_GetTransferList in g.mtpDeviceInfo.operationsSupportedSet or g.cameraMake == CAMERA_MAKE_NIKON:
            try:
                ...
                applog_i("Transfer list detected in camera - {:d} images/file(s) will be downloaded".format(
    ...
    if isDebugLog():
        ...
        applog_d(strutil.hexdump(struct.pack('<' + 'I' * countFullObjHandlesList, *fullObjHandlesList), bytesPerField=4,
                                 includeASCII=False))
        if g.fAllObjsAreFromCameraTransferList:
            applog_d("Transfer List MTP object handles (count={:d}):".format(numObjectHandlesToGet))
            applog_d(
                strutil.hexdump(struct.pack('<' + 'I' * numObjectHandlesToGet, *objHandlesListToGet), bytesPerField=4,
                                includeASCII=False))
    ...
    if fReportObjectTotals or isDebugLog():  # always report object totals when debug logging is enabled
        applog_i("Processed info for {:d} files/dirs [{:d} from object cache, {:d} previous]".format( \
            createMtpObjectStatsStruct.countObjectsProcessed, createMtpObjectStatsStruct.countCacheHits,
    ...
    if g.downloadHistoryDict:
        ...
        if (os.path.exists(downloadHistoryFilename)):
            if g.args['downloadhistory'] == 'clear':
                ...
                #
                applog_v(
                    "Deleting download history file \"{:s}\" per user configuration".format(downloadHistoryFilename))
                ...
                try:
                    with open(downloadHistoryFilename) as f:
                        downloadHistoryDict_Temp = {"{:s}".format(line[0:line.index("::::")]): "{:s}".format(
                            line[line.index("::::", 0) + 4: line.rindex("\n")]) for line in f}
                    # succesfully loaded history
                    downloadHistoryDict = downloadHistoryDict_Temp
                    applog_v("Download history file \"{:s}\" loaded - {:d} entries".format(downloadHistoryFilename,
                                                                                           len(downloadHistoryDict)))
                except ValueError:
                    applog_e("Download history file \"{:s}\" appears corrupt and will be ignored and deleted".format(
...
def doesMtpObjectPassUserFileFilter(mtpObject, fPrintFilterAction=True):
    ...
    if mtpObject.mtpObjectInfo.objectFormat == MTP_OBJFORMAT_Assocation or mtpObject.mtpObjectInfo.objectFormat == MTP_OBJFORMAT_NONE:
        if fPrintFilterAction:
            applog_d("Skipping {:s} - object is not file - {:s}".format(mtpObject.mtpObjectInfo.filename,
                                                                        getMtpObjFormatDesc(
                                                                            mtpObject.mtpObjectInfo.objectFormat)))
        return False
    ...
    if g.args['extlist'] and isMtpFilenameExtInList(mtpObject.mtpObjectInfo.filename, g.args['extlist']) == False:
        if fPrintFilterAction:
            applog_v("Skipping {:s} - filename extension not in user-specified list".format(
    ...
    if g.objfilter_dateStartEpoch != None and mtpObject.captureDateEpoch < g.objfilter_dateStartEpoch:
        ...
        if fPrintFilterAction:
            applog_v("Skipping {:s} - has capture date earlier than user-specified start date filter".format(
                mtpObject.mtpObjectInfo.filename))
        return False
    if g.objfilter_dateEndEpoch != None and mtpObject.captureDateEpoch > g.objfilter_dateEndEpoch:
        ...
        if fPrintFilterAction:
            applog_v("Skipping {:s} - has capture date later than user-specified end date filter".format(
                mtpObject.mtpObjectInfo.filename))
        return False
    ...
    if g.args['onlyfolders']:
        ...
        if (cameraFolder == "" and ("<ROOT>" not in g.args['onlyfolders'])) or cameraFolder not in g.args[
            ...
            if fPrintFilterAction:
                applog_v("Skipping {:s}\\{:s} - folder not in --onlyfolders".format(cameraFolder,
                                                                                    mtpObject.mtpObjectInfo.filename))
            return False
    if g.args['excludefolders']:
        ...
        if (cameraFolder == "" and "<ROOT>" in g.args['excludefolders']) or cameraFolder in g.args['excludefolders']:
            ...
            if fPrintFilterAction:
                applog_v("Skipping {:s}\\{:s} - folder in --excludefolders".format(cameraFolder,
                                                                                   mtpObject.mtpObjectInfo.filename))
            return False
...
def genRenameDictKeysCommonToAllMtpObjects():
    ...
    renameDict['cameramake'] = g.mtpDeviceInfo.manufacturerStr.split(' ')[
        0]  # just use the first word. for example, Nikon returns "Nikon Corporation"
...
#
def updateRenameDictKeysSpecificToMtpObject(renameDict, mtpObject, dlnum, dlnum_lifetime, localFilename=None,
                                            path=None):
...
def doDownloadExec(renameDict, mtpObject):
    if g.args['downloadexec_extlist'] and isMtpFilenameExtInList(mtpObject.mtpObjectInfo.filename,
                                                                 g.args['downloadexec_extlist']) == False:
        return
    ...
    for arg in g.args['downloadexec']:
        ...
        if renamedArg != "":
            ...
            if not execArgs:
                ...
                applog_v("Skipping 'downloadexec' for \"{:s}\" due to empty first argument of rename string".format(
                    renameDict['filename']))
                break;
    ...
    if execArgs:
        ...
        try:
            ...
            errStr = "Error launching for 'downloadexec' {:s}: {:s} {:s}".format(str(execArgs), str(sys.exc_info()[0]),
                                                                                 str(sys.exc_info()[1]))
        ...
        if 'wait' in g.args['downloadexec_options']:
            ...
            if 'exitonfailcode' in g.args['downloadexec_options']:
                if retCode != 0:
                    applog_i("Exiting due to non-zero return code ({:d}) of launched app for '--downloadexec'".format(
                        retCode))
...
def respErrorDuringDownload_checkIfFileDeletedOnCamera(mtpObject, localFilename, mtpOpExecFailureException):
    ...
    #
    applog_d("RESP error downloading \"{:s}\": {:s}".format(localFilename,
                                                            getMtpRespDesc(mtpOpExecFailureException.mtpRespCode)))
    try:
        ...
        mtpRespCodesIndicatingFileDeleted = [MTP_RESP_InvalidObjectHandle, MTP_RESP_OperationNotSupported,
                                             MTP_RESP_ParameterNotSupported, \
                                             MTP_RESP_AccessDenied, MTP_RESP_PartialDeletion,
                                             MTP_RESP_NoValidObjectInfo, MTP_RESP_InvalidParentObject,
...
#
def downloadMtpFileObjects_DownloadProgressCallback(bytesReceivedAllCurrentPayloads,
                                                    totalBytesExpectedAllCurrentPayloads, bytesReceivedPriorPayloads,
...
def downloadMtpFileObjects(firstMtpObjectToDownload=None):
    ...
    while True:
        ...
        if fUsingRenameEngineForAnyParameter:
            ...
            updateRenameDictKeysSpecificToMtpObject(renameDict, mtpObject,
                                                    g.countFilesDownloadedPersistentAcrossStatsReset,
                                                    len(downloadHistoryDict), filenameWithObjTypeSuffixBeforeRename)
        ...
        #
        downloadHistoryDescStr_Key = "{:s}::{:s}::{:,}".format(filenameWithObjTypeSuffixBeforeRename,
        ...
        if fSkipDownloadedFiles and downloadHistoryDescStr_Key in downloadHistoryDict:
            ...
            listDownloadHistory = str.split(downloadHistoryDict[downloadHistoryDescStr_Key],
            ...
            applog_v("Skipping \"{:s}\" - downloaded on {:s} to \"{:s}\" ". \
                     format(filenameWithObjTypeSuffixBeforeRename, downloadHistoryElementTuple.dateDownloadedStr,
        ...
        if mtpObject.partialDownloadObj().getLocalFilenameWithoutPath() == None:
            ...
            if (os.path.exists(localFilenameWithPath)):
                if g.args['ifexists'] == 'prompt':
                    ...
                    #
                    keyResponse = promptWithSingleKeyResponse("(S)kip, (O)verwrite, (U)niquename, (E)xit [+enter]: ",
                                                              'soue')
                else:
                    keyResponse = ''
                if g.args['ifexists'] == 'skip' or keyResponse == 'S':
                    ...
                    if not keyResponse:
                        applog_v("\"{:s}\" exists - will write to \"{:s}\"".format(localFilenameWithPath,
                                                                                   uniqueFilenameWithoutPath))
        ...
        if g.fAllObjsAreFromCameraTransferList:
            mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_NotifyFileAcquisitionStart,
        ...
        if bUsingGetPartialObject:
            ...
            try:
                ...
                if bytesWritten > 0:
                    ...
                    #
                    applog_d("{:s} - resuming download - bytesWritten=0x{:x}, fileSize=0x{:x}".format(
                ...
                while offsetIntoImage < fileSizeBytes:
                    ...
                    if len(dataReceived) + bytesToDownloadThisPiece > g.maxGetObjBufferSize:
                        ...
                        bytesToDownloadThisPiece -= (len(
                            dataReceived) + bytesToDownloadThisPiece) - g.maxGetObjBufferSize
                    if bytesToDownloadThisPiece == 0:
                        ...
                        raise AssertionError(
                            "bytesToDownloadThisPiece is zero! offset=0x{:x}, fileSize=0x{:x}, dataReceived=0x{:x}, max=0x{:x}/0x{:x}".format( \
                                offsetIntoImage, fileSizeBytes, len(dataReceived), g.maxGetObjTransferSize,
                                g.maxGetObjBufferSize))
                    applog_d(
                        "{:s} - downloading next piece, offset=0x{:x}, count=0x{:x}".format(localFilenameWithoutPath,
                                                                                            offsetIntoImage,
                                                                                            bytesToDownloadThisPiece))
                    timeStart = secondsElapsed(None)
                    mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_GetPartialObject,
                                                              struct.pack('<III', \
                                                                          mtpObject.mtpObjectHandle, offsetIntoImage,
                                                                          bytesToDownloadThisPiece), \
                                                              rxTxProgressFunc=lambda bytesReceivedAllCurrentPayloads,
                                                                                      totalBytesExpectedAllCurrentPayloads: \
                                                                  downloadMtpFileObjects_DownloadProgressCallback(
                                                                      bytesReceivedAllCurrentPayloads,
                                                                      totalBytesExpectedAllCurrentPayloads,
                                                                      offsetIntoImage, fileSizeBytes))
                ...
                if e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR and respErrorDuringDownload_checkIfFileDeletedOnCamera(
                        mtpObject, localFilenameWithoutPath, e):
                ...
                if fFileDeletedOnCamera == False:
                    ...
                    mtpObject.partialDownloadObj().addDownloadTimeSecs(secondsElapsed(timeStart))
                    applog_d("{:s} - error during download, writing 0x{:x} bytes of buffered data".format(
                        localFilenameWithoutPath, len(dataReceived)))

                    # flush out any unwritten data we have in 'dataReceived'
                    if dataReceived:
                        foDownloadedFile = writeDataToDownloadedFile(foDownloadedFile, localFilenameWithPath,
                                                                     dataReceived, False, (bytesWritten != 0))
                        bytesWritten += len(dataReceived)
                        mtpObject.partialDownloadObj().addBytesWritten(len(dataReceived))
                    if e.partialData:
                        #
                        ...
                        #
                        applog_d("{:s} - writing partial payload data of 0x{:x} bytes".format(localFilenameWithoutPath,
                                                                                              len(e.partialData)))
                        foDownloadedFile = writeDataToDownloadedFile(foDownloadedFile, localFilenameWithPath,
                                                                     e.partialData, False, (bytesWritten != 0))
            ...
            timeStart = secondsElapsed(None)
            try:
                mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(g.socketPrimary, mtpOpGet,
                                                          struct.pack('<I', mtpObject.mtpObjectHandle), \
                                                          rxTxProgressFunc=lambda bytesReceivedAllCurrentPayloads,
                                                                                  totalBytesExpectedAllCurrentPayloads: downloadMtpFileObjects_DownloadProgressCallback(
                                                              bytesReceivedAllCurrentPayloads,
                                                              totalBytesExpectedAllCurrentPayloads, 0, 0))
                dataReceived = mtpTcpCmdResultGetObj.dataReceived
        ...
        if not fFileDeletedOnCamera:
            ...
            os.rename(localFilenameWithPath_TemporaryFilename,
                      localFilenameWithPath)  # download done - safe to rename to final filename
            ...
            applog_i("{:s} [size = {:,}] in {:.2f} seconds ({:.2f} MB/s)".format(localFilenameWithPath, \
                                                                                 fileSizeBytes, fileDownloadTimeSecs,
                                                                                 thisFileDownloadRateMbSec))
            ...
            #
            if g.fAllObjsAreFromCameraTransferList:
                mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_NotifyFileAcquisitionEnd,
                                  struct.pack('<I', mtpObject.mtpObjectHandle))
            ...
            if g.args['downloadexec']:
                renameDict[
                    'filename'] = localFilenameWithoutPath  # update dict in case filename changed due to ifexists unique creation
...
def printMtpObjectDirectoryListing():
    ...
    while mtpObject:
        ...
        if g.countCardsUsed > 1:
            ...
            fullPathStr = "CARD{:d}\\".format(
                getSlotIndexFromStorageId(mtpObject.mtpObjectInfo.storageId)) + fullPathStr
    ...
    applog_i("        {:4d} Dir(s)  {:13,} bytes free {:s}".format(MtpObject._CountMtpObjectDirectories,
                                                                   g.mtpStorageInfoList[0].freeSpaceBytes, \
                                                                   "[CARD 1]" if g.countCardsUsed > 1 else ""))
    for cardIndex in xrange(1, g.countCardsUsed):
        applog_i(
            "                     {:13,} bytes free [CARD {:d}]".format(g.mtpStorageInfoList[cardIndex].freeSpaceBytes,
                                                                        cardIndex + 1))
...
def getMtpDeviceProperty(mtpDevicePropCode, fIgnoreIfNotSupported=False):
    try:
        mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_GetDevicePropValue,
                                                  struct.pack('<I', mtpDevicePropCode))
...
def setMtpDeviceProperty(mtpDevicePropCode, devicePropData, fIgnoreIfNotSupported=False):
    try:
        mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_SetDevicePropValue,
                                                  struct.pack('<I', mtpDevicePropCode), devicePropData)
        return False
    except mtpwifi.MtpOpExecFailureException as e:
        if fIgnoreIfNotSupported and e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR:
            return True
...
def setMtpDeviceProperty_Canon(devicePropData, fIgnoreIfNotSupported=False):
    try:
        mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(g.socketPrimary, MTP_OP_Canon_SetDevicePropValue,
                                                  dataToSend=devicePropData)
        return False
    except mtpwifi.MtpOpExecFailureException as e:
        if fIgnoreIfNotSupported and e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR:
            return True
...
def syncCameraDateTimeIfNecessary():
    ...
    if g.cameraMake != CAMERA_MAKE_CANON:  # we don't suppor retriving current time on Canon bodies, so we always set it unconditionally
        ...
        applog_d("Camera time epoch: {:.2f}, System time epoch: {:.2f}".format(cameraCurrentTimeEpoch,
...
def printSpinningProgressCharToConsole():
    ...
    if not hasattr(printSpinningProgressCharToConsole,
                   "lastProgressCharPrinted"):  # static variable to track last progress character printed
...
def genNikonEventListDescription(mtpEventList):
    ...
    for nthEvent in xrange(len(mtpEventList)):
        ...
        if nthEvent > 0:
            str += "\n"
        str += "E#{:d}: {:s}, Param: {:s}".format(nthEvent, getMtpEventDesc(mtpEventList[nthEvent].eventCode),
                                                  parameterDesc)
...
def realTimeCapture_NikonEventsMethod():
    ...
    try:
                ...
                for nthEvent in xrange(len(nikonMtpEventList)):
                    if nikonMtpEventList[nthEvent].eventCode == MTP_EVENT_ObjectAdded:
                        ...
                        if MtpObject.getByMtpObjectHandle(objHandle):
                            applog_d("realTimeCapture: MtpObject for handle 0x{:08x} already exists, skipping".format(
                                objHandle))
                            continue
                        mtpObjectInfo = getMtpObjectInfo(objHandle)
                        if g.storageId != MTP_STORAGEID_ALL_CARDS and mtpObjectInfo.storageId != g.storageId:
                            if isVerboseLog():
                                consoleClearLine()
                                fRedrawWaitingMessage = True
                                applog_v("Ignoring \"{:s}\" because it's not from your configured --slot".format(
        ...
        consoleClearLine()
        g.dlstats.reportDownloadStats(g.args[
                                          'realtimedownload'] != 'only')  # print stats even if no files downloaded only if this was a realtime-only session
        raise
...
def realTimeCapture_MtpObjPollingMethod():
    ...
    if not lastFullMtpHandleList:
        ...
        if isDebugLog():
            applog_d("realTimeCapture_MtpObjPollingMethod(): First MTP object list (count={:d}):".format(
                len(lastFullMtpHandleList)))
            applog_d(strutil.hexdump(struct.pack('<' + 'I' * len(lastFullMtpHandleList), *lastFullMtpHandleList),
                                     bytesPerField=4, includeASCII=False))
    ...
    try:
        while True:
            ...
            if g.args['rtd_mtppollingmethod_newobjdetection'] == 'numobjs':
                ...
                if fNumObjsChanged:
                    applog_d("fNumObjsChanged TRUE: (previous=0x{:d}, new=0x{:d}".format(len(lastFullMtpHandleList),
                                                                                         numMtpObjects))
            ...
            if fNumObjsChanged or g.args['rtd_mtppollingmethod_newobjdetection'] == 'objlist':
                ...
                if isDebugLog() and (fNumObjsChanged or newMtpHandleList):
                    applog_d("realTimeCapture_MtpObjPollingMethod(): Current MTP object list (count={:d}):".format(
                        len(currentFullMtpHandleList)))
                    applog_d(strutil.hexdump(
                        struct.pack('<' + 'I' * len(currentFullMtpHandleList), *currentFullMtpHandleList),
                        bytesPerField=4, includeASCII=False))
                    applog_d("realTimeCapture_MtpObjPollingMethod(): New MTP object list (count={:d}):".format(
                        len(newMtpHandleList)))
                    applog_d(strutil.hexdump(struct.pack('<' + 'I' * len(newMtpHandleList), *newMtpHandleList),
                                             bytesPerField=4, includeASCII=False))
...
def realTimeCapture():
    ...
    g.args[
        'cameratransferlist'] = 'ignore'  # so that recovery operations use the full camera object list rather than any transfer list already processed
    g.fAllObjsAreFromCameraTransferList = False  # we'll be adding non-transfer list objects (ie, the files we download in realtime)
    g.args[
        'transferorder'] = 'oldestfirst'  # so that downloadMtpFileObjects() will properly enumerate through multiple realtime images as we add them
    ...
    try:
        ...
        if g.args['action'] == 'listfiles':
            ...
            if g.fRetrievedMtpObjects:
                ...
                downloadMtpFileObjects()
                g.dlstats.reportDownloadStats(g.fRealTimeDownloadPhaseStarted or g.args[
                    'realtimedownload'] == 'only')  # don't print stats if no files downloaded if we're in realtime phase or in a session recovery of a realtime-only session
        ...
        if e.mtpRespCode == MTP_RESP_COMMUNICATION_ERROR:
            ...
            return (ERRNO_CAMERA_UNEXPECTED_RESP_CODE,
                    False)  # don't retry on a high-level MTP error - we're trying to do something the camera doesn't like
...
#
def shutdownApplog():
    applog(">>>> airnefcmd session over - App Exit Time: {:s}".format(strutil.getDateTimeStr(fMilitaryTime=True)),
           APPLOGF_LEVEL_INFORMATIONAL | APPLOGF_DONT_WRITE_TO_CONSOLE)  # used by utils to see if we exited gracefully
    applog_shutdown()
...
def main():
    ...
    #
    _errno = applog_init(APPLOGF_LEVEL_INFORMATIONAL | APPLOGF_LEVEL_ERROR,
                         os.path.join(g.appDataDir, "airnefcmd-log-last.txt"), \
    ...
    while True:
        try:
            ...
            consoleWriteLine("\rDelaying {:d} seconds before retrying. Press <ctrl-c> to exit [{:d} attempts]: ".format(
            ...
            while secondsToNextRetry:
                ...
                time.sleep(1)
                consoleWriteLine("\b" * len(secondsToNextRetryStr) + " " * len(secondsToNextRetryStr) + "\b" * len(
                    secondsToNextRetryStr))
