...
def rxPayload(s, rxProgressFunc=None):
    ...
    try:
        ...
        if payloadBytesReceived and (
                        payloadId == MTP_TCPIP_PAYLOAD_ID_DataPayload or payloadId == MTP_TCPIP_PAYLOAD_ID_DataPayloadLast) and payloadBytesReceived >= 12:
            #
...
#
def execMtpOp_rxPayloadProgressFunc(totalBytesReceivedThisPayload, rxTxProgressFunc,
...
def execMtpOp(s, mtpOp, cmdArgsPacked=six.binary_type(), dataToSend=six.binary_type(), rxTxProgressFunc=None):
    ...
    theCmdReq = struct.pack('<IIHI', MTP_TCPIP_PAYLOAD_ID_CmdReq, mtpDataDirToCmdReqDataDirectionCode[dataDirection],
                            mtpOp, txTransactionId) + cmdArgsPacked
    ...
    while True:
        ...
        try:
            ...
            data = rxPayload(s, lambda totalBytesReceivedThisPayload: execMtpOp_rxPayloadProgressFunc(
            ...
            if payloadId == MTP_TCPIP_PAYLOAD_ID_DataStart:
                ...
                if isDebugLog():
                    applog_d("execMtpOp: {:s} - DataStart payload [expected data bytes is 0x{:x}]".format(
                ...
                if isDebugLog():
                    if mtpOp == MTP_OP_GetObject or mtpOp == MTP_OP_GetPartialObject or mtpOp == MTP_OP_GetLargeThumb or mtpOp == MTP_OP_GetThumb:
                        ...
                        maxBytesToDump = 4096
                    applog_d("execMtpOp: {:s} - Data payload [ID {:x}] (0x{:08x} bytes):".format(getMtpOpDesc(mtpOp),
                                                                                                 payloadId, len(data)))
                ...
                if isDebugLog():
                    applog_d("execMtpOp: {:s} - CmdResponse payload (resp=\"{:s}\"):".format(getMtpOpDesc(mtpOp),
                                                                                             getMtpRespDesc(
                                                                                                 mtpRespCode)))
                    applog_d(strutil.hexdump(data))

                if mtpRespCode != MTP_RESP_Ok:
                    raise MtpOpExecFailureException(mtpRespCode, "Camera Command Failed: {:s}, Error: {:s}".format(
                ...
                #
                if totalDataTransferSizeBytesExpectedAcrossAllPayloads and len(
                ...
                if isDebugLog():
                    ...
                    applog_d(strutil.hexdump(data))
                raise MtpProtocolException(
                    "Camera Networking Error: {:s}: Unrecognized payload ID (0x{:08x})".format(getMtpOpDesc(mtpOp),
                                                                                               payloadId))
            ...
            if g_PartialRxDataPayloadData:
                ...
                if isDebugLog():
                    applog_d(
                        "execMtpOp: {:s} - Partial payload after error (0x{:08x} bytes):".format(getMtpOpDesc(mtpOp),
                                                                                                 len(data)))
                    applog_d(strutil.hexdump(data[:min(len(data), 1024)]))
                (rxTransactionId,) = struct.unpack('<I', data[4:8])
                if rxTransactionId != txTransactionId:
            ...
            raise MtpOpExecFailureException(MTP_RESP_COMMUNICATION_ERROR, \
                                            ...
                                            format(getMtpOpDesc(mtpOp), bytesReceivedLastPayload,
                                                   lastPayloadExpectedSize, len(dataReceivedSoFar),
                                                   totalDataTransferSizeBytesExpectedAcrossAllPayloads, str(e)),
                                            dataReceivedSoFar, totalDataTransferSizeBytesExpectedAcrossAllPayloads)
...
def sendInitCmdReq(s, guidHighLowTuple, hostNameStr, hostVerInt):
    ...
    try:
        ...
        if wordResponse == 0x2 and len(
                rxdata) >= 8:  # make sure first 32-bit word is equal to a value of 0x2 ("ACK") and has 4-byte session ID after
            return rxdata[4:]
        else:
            raise MtpProtocolException( \
...
#		
def openConnection(ipAddrStr, verbose, connectionTimeoutSecs=SOCKET_TIMEOUT_CONNECT_SECS_DEFAULT,
