...
RealtimeTransferUserReadableChoices = ['none - exit after normal download', 'normal download then realtime',
                                       'only realtime download']
RealtimeTransferUserReadableChoicesToAirnefCmdOtion = {
    # converts a RealtimeTransferUserReadableChoices choice to the equivalent airnefcmd command line option
...
IfExistsUserReadableChoices = ['generate unique filename', 'overwrite file', 'skip file', 'prompt for each file',
                               'exit']
IfExistsUserReadableChoicesToAirnefCmdOption = {
    # converts a IfExistsUserReadableChoices choice to the equivalent airnefcmd command line option
...
ActionsUserReadableChoicesToAirnefCmdOption = {
    # converts a ActionsUserReadableChoices choice to the equivalent airnefcmd command line option
...
CardSlotUserReadableChoicesToAirnefCmdOption = {
    # converts a CardSlotUserReadableChoices choice to the equivalent airnefcmd command line option
...
TransferOrderReadableChoicesToAirnefCmdOption = {
    # converts a TransferOrderReadableChoices choice to the equivalent airnefcmd command line option
            ...
            try:
                ...
                if e.errno == errno.ESRCH:
                    if not os.path.exists(
                            filename):  # exclude race condition between us checking file vs process ID (only for print purposes - doesn't really matter otherwise)
...
def osxFrozenWorkaround_LaunchAirnefcmdWrapper(argStr):
    FILENAME_CMD_OPTS = os.path.join(g.appDataDir,
                                     "airnefcmd-osxfrozen-cmdopts")  # file we encode options for the wrapper to read
    FILENAME_NOTIFY_START = os.path.join(g.appDataDir,
                                         "airnefcmd-osxfrozen-notifystart")  # file created by wrapper to signal it's started running
    FILENAME_NOTIFY_DONE = os.path.join(g.appDataDir,
                                        "airnefcmd-osxfrozen-notifydone")  # file created by wrapper to signal when airnefcmd is done
    ...
    process = subprocess.Popen(
        ['open', '-a', 'Terminal.app', os.path.join(g.appDir, 'airnefcmd_OSX_Frozen_Wrapper.py')])
...
def displayAirnefcmdOutput():
    ...
    try:
        ...
        tkMessageBox.showerror("airnef Transfer Report", "Error reading airnefcmd report file \"{:s}\". {:s}".format(
            mostRecentAirnefReportFilename, str(e)))
        return
    finally:
        if fileReport:
            fileReport.close()
    ...
    scrolledText.bind("<Control-c>", lambda
        e: "")  # on ctrl-c, invoke function that returns empty string, allowing the default ahndler to process the ctrl-c copy operations
    scrolledText.bind('<Key>', lambda
        e: "break")  # on all other keys, invoke function that returns "break", preventing the keypress from being handled
    ...
    button.focus_set()
    button = Button(buttonFrame, text="Copy to Clipboard",
                    command=lambda: [root.clipboard_clear(), root.clipboard_append(reportContents)])
...
class Application(Frame):
    ...
    def __init__(self, master):
        ...
        button = Button(self.toolbarFrame, text="Last Transfer Report",
                        command=lambda: self.toolbarClick('transfer_report'))
    ...
    def loadAppConfig(self):
        ...
        try:
            ...
            tkMessageBox.showwarning("Loading App Config",
                                     "Could not read app config data. {:s}. Defaults will be used.".format(str(e)))
        except ValueError as e:
            tkMessageBox.showwarning("Loading App Config",
                                     "Could not decode app config data. {:s}. Defaults will be used.")
    ...
    def setContent_Wizard_0(self):
        ...
        button = Button(leftFrame, image=self.getResource_Image("camera_button_200x134.gif"), compound=TOP,
                        text="Select in Camera", bg=buttonBgColor,
                        command=lambda: self.wizard_0_ButtonClick('select_in_camera'))
        ...
        button = Button(rightFrame, image=self.getResource_Image("computer_200x134.gif"), compound=TOP,
                        text="Select on Computer", bg=buttonBgColor,
                        command=lambda: self.wizard_0_ButtonClick('select_in_computer'))
    ...
    def createSkipFilesInDownloadHistoryCheckbox(self, parentFrame):
        ...
        if 'skip_files_in_download_history_choice' in self.appConfigDict:
            self.wizard_SkipFilesInDownloadHistoryCheckboxIntVar.set(
                self.appConfigDict['skip_files_in_download_history_choice'])
        else:
            self.wizard_SkipFilesInDownloadHistoryCheckboxIntVar.set(True)
        checkButton = Checkbutton(parentFrame, variable=self.wizard_SkipFilesInDownloadHistoryCheckboxIntVar,
                                  onvalue=True, text="Skip images/movies you've previously downloaded")
        return checkButton
    ...
    def createStartDownloadFrameAndButton_PackOnRightInParent(self, parentFrame, theCommand, buttonPady):
        rightFrame = Frame(self.contentFrame, bg=mainBgColor)
        button = Button(rightFrame, image=self.getResource_Image("wifi_200x134.gif"), compound=TOP,
                        text="Start Download", bg=buttonBgColor, command=theCommand)
        button.pack(side=TOP, fill=BOTH, expand=1, ipadx=40, ipady=40 if not g.isOSX else 10, padx=40,
                    pady=buttonPady)  # note001
    ...
    def setContent_Wizard_1_0(self):
        ...
        label = Label(leftFrame,
                      text="1. Turn your camera on and select which images you'd like to upload. This may be under the " \
                           "camera's WiFi menu as \"Select to send to smart device\" or in the playback menu, depending on the camera model.",
                      bg=mainBgColor, wraplength=300, justify=LEFT)
        label.grid(column=0, columnspan=3, row=0, sticky=W, padx=5, pady=5)
        label = Label(leftFrame,
                      text="2. Make sure your Camera's WiFi is on. The camera may sometimes disable it without direction from the user.",
                      bg=mainBgColor, wraplength=300, justify=LEFT)
        label.grid(column=0, columnspan=3, row=1, sticky=W, padx=5, pady=5)
        label = Label(leftFrame,
                      text="3. Join your Camera's WiFi network on your computer. It will have a name prefixed with \"Nikon\", such as \"Nikon_WU2_0090B6245A45\".",
                      bg=mainBgColor, wraplength=300, justify=LEFT)
        ...
        self.createStartDownloadFrameAndButton_PackOnRightInParent(self.contentFrame,
                                                                   lambda: self.wizard_1_0_ButtonClick(
                                                                       'start_download_selected_in_camera'),
    ...
    def wizard_1_EnableBackNavigationButton(self):
        if not self.wizard_BackNavigationToolbarButton:
            self.wizard_BackNavigationToolbarButton = Button(self.toolbarFrame, text="Prev <", fg="red",
                                                             command=lambda: self.toolbarClick('Wizard_1_Prev'))
    ...
    def wizard_1_0_ButtonClick(self, str):
        if str == 'start_download_selected_in_camera':
            ...
            self.appConfigDict[
                'skip_files_in_download_history_choice'] = self.wizard_SkipFilesInDownloadHistoryCheckboxIntVar.get()
            ...
            argStr += " " + RealtimeTransferUserReadableChoicesToAirnefCmdOtion[
                self.toolbar_RealtimeDownloadComboBox.get()]
    ...
    def setContent_Wizard_2_0(self):
        ...
        # button to start download
        rightFrame = self.createStartDownloadFrameAndButton_PackOnRightInParent(self.contentFrame,
                                                                                lambda: self.wizard_2_0_ButtonClick(
                                                                                    'start_download_selected_on_computer'),
                                                                                (35, 10))

        # button to preview file list
        button = Button(rightFrame, text="Preview File List for Criteria",
                        command=lambda: self.wizard_2_0_ButtonClick('preview_file_list'))
    ...
    def wizard_2_0_ButtonClick(self, str):
        ...
        if str == 'start_download_selected_on_computer' or str == 'preview_file_list':
            ...
            if (not fileExtList) and (not fileExtMore):
                tkMessageBox.showwarning("Selection Issue",
                                         "At least one file type must be checked/entered, otherwise there wont be anything to download :)")
                return;
            ...
            if self.wizard_2_0_DateSelectionComboBox.get() == 'custom date range':
                ...
                if dateStr:
                    if (Application.validateCustomDateEntry(dateStr)):
                        tkMessageBox.showwarning("Invalid Start Date",
                                                 "Start date specified \"{:s}\" is formatted incorrectly or has an invalid date/time. It must be formatted as mm/dd/yy or mm/dd/yy hh:mm:ss (including leading zeros) and be a valid date/time.". \
                                                 format(dateStr))
                        return
                dateStr = self.wizard_2_0_CustomDateRangeEndEntry.get()
                if dateStr:
                    if (Application.validateCustomDateEntry(dateStr)):
                        tkMessageBox.showwarning("Invalid End Date",
                                                 "End date specified \"{:s}\" is formatted incorrectly or has an invalid date/time. It must be formatted as mm/dd/yy or mm/dd/yy hh:mm:ss (including leading zeros) and be a valid date/time.". \
                                                 format(dateStr))
                        return
            ...
            if dateChoiceStr == 'All Dates':
                ...
                if dateTimeForUserSpecifiedInterval != None:
                    argStr += " --startdate {:02d}/{:02d}/{:02d}".format(dateTimeForUserSpecifiedInterval.month,
                                                                         dateTimeForUserSpecifiedInterval.day,
                                                                         dateTimeForUserSpecifiedInterval.year - 2000)
...
def verifyPythonVersion():
    if sys.version_info.major == 2:
        if sys.version_info.minor < 7:
            ...
            print("Version running is {:d}.{:d}.{:d}, app was tested on 2.7.x".format(sys.version_info.major,
                                                                                      sys.version_info.minor,
                                                                                      sys.version_info.micro))
    elif sys.version_info.major == 3:
        if sys.version_info.minor < 4:
            print("Warning: You are running a Python 3.x version older than app was tested with.")
            print("Version running is {:d}.{:d}.{:d}, app was tested on 3.4.x".format(sys.version_info.major,
                                                                                      sys.version_info.minor,
                                                                                      sys.version_info.micro))
...
def main():
    ...
    if not os.path.exists(g.appResourceDir):
        tkMessageBox.showwarning("Setup Error",
                                 "The appresource subdirectory is missing. This directory and its files (icons/bitmaps/etc..) are needed for proper functioning of airnef :(")
        return
