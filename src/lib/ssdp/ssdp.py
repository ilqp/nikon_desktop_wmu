...
#
def discover(serviceNameList, ssdpServiceVerifyOkCallback=None, numAttempts=3, timeoutSecsPerAttempt=2, flags=None,
    ...
    for nthAttempt in xrange(numAttempts):
        ...
        try:
            ...
            while True:
                ...
                (socketsWithRxData, socketsReadyTxData, socketsError) = select.select(sockList, [], [], float(
                    timeoutSecsPerAttempt) - elapsedTimeThisAttemptSecs)
...
def createUdpSocket(discoverFlags):
    ...
    sock.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL,
                    31 if (discoverFlags & SSDP_DISCOVERF_USE_TTL_31) else 1)
    return sock
...
def getHeader(ssdpMessage, headerNameStr):
    ...
    for line in messageLines:
        ...
        if line.upper().startswith(
                headerNameStr):  # field names are case-insensitive, so both are compared in uppercase
